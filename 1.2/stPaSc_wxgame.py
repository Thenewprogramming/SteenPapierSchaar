...
class SecondaryFrame(wx.Frame):
	def __init__(self, parent, id, user_name1, user_name2, server, s, conn, client):
		wx.Frame.__init__(self, parent, id, "rock, paper, scissors - Play a game", pos=(150, 150), size=(400, 300))
		...
		self.rockBtn = wx.Button(self.playPanel, -1, "rock", pos=(10, 100), size=(100 , -1))
		self.paperBtn = wx.Button(self.playPanel, -1, "paper", pos=(142.5, 100), size=(100 , -1))
		self.scissorsBtn = wx.Button(self.playPanel, -1, "scissors", pos=(275, 100), size=(100 , -1))

		self.Bind(wx.EVT_BUTTON, self.choserock, self.rockBtn)
		self.Bind(wx.EVT_BUTTON, self.chosepaper, self.paperBtn)
		self.Bind(wx.EVT_BUTTON, self.chosescissors, self.scissorsBtn)
	...
	def choserock(self,event):
		self.choice1 = "rock"
		self.givescores()

	def chosepaper(self, event):
		self.choice1 = "paper"
		self.givescores()

	def chosescissors(self, event):
		self.choice1 = "scissors"
		self.givescores()

	def givescores(self):
		self.rockBtn.Hide()
		self.paperBtn.Hide()
		self.scissorsBtn.Hide()
	...
	def getWinner(self):
		if self.server:
			if self.choice1 in ["1","rock"]:
				self.choice1_n = 1
			elif self.choice1 in ["2","paper"]:
				self.choice1_n = 2
			elif self.choice1 in ["3","scissors"]:
				self.choice1_n = 3

			if self.choice2 in ["1","rock"]:
				self.choice2_n = 1
			elif self.choice2 in ["2","paper"]:
				
				self.choice2_n = 2
			elif self.choice2 in ["3","scissors"]:
				self.choice2_n = 3
...
class MainFrame(wx.Frame):
	...
	def __init__(self, parent, id):
		wx.Frame.__init__(self, parent, id, "rock, paper, scissors - Connect to Player", pos=(150, 150), size=(400, 300))
